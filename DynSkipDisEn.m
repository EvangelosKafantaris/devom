function [Out_DispEn, npdf]=DynSkipDisEn(x,m,nc,MA,tau,cutoff)
%% This is an implementation of the Dynamic Skip Sample Dispersion Entropy
% (DynSkipDisEn)which is a variation of the original Dispersion Entropy algorithm.

% The DynSkipDisEn variation implements a dynamic skipping approach through
% the use of an additional parameter named cutoff, automatically discarding
% any samples with values that deviate more than a certain factor, defined by
% the cutoff parameter, of standard deviations from the mean of each analyzed signal segment.
...
% The following line introduces a sample skipping mechanism for the case of
% missing samples such as the one used in the SkipDisEn variation  
x = x(~isnan(x));
...
% Dealing with outlier values, any value that is
% Outside "cutoff" standard deviations from the mean is removed
x = x(x < mu_x+ cutoff*sigma_x & x > mu_x-cutoff*sigma_x);
